...
int main () {
    ...
    while ( ( front = std::fgetc ( stdin ) ) != EOF ) { // read in CFG productions
    ...
    for ( int i = 0; ! workQueue.empty(); ++i ) {
        ...
        printf ( "I%d:\n", i );
        for ( auto& j : workQueue.front() ) { // j ==  production STR_NUM_PAIR from the the current item list
            ...
            if ( itgRetr.second == false ) // yes it is mapped
                printf ( "   %-20s goto(%c)=I%d\n", j.first.c_str(), front, itgRetr.first->second );
            else { // no it isn't mapped
            ...
            if ( isupper ( front ) ) {
                ...
                while ( ! closureQueue.empty() ) {
                    for ( auto k : closureMap [ closureQueue.front() ] ) { // k == production STR_NUM_PAIR from closure list from closure map
                        front = k.first [ k.second ];
                        uniqueNT = false;
                        if ( closureDone.find ( front ) == closureDone.end() ) { // new NT symbol
                            uniqueNT = true;
                            closureDone.insert ( front ); // add to closure done queue
                            closureQueue.push ( front ); // push in closure queue
                        }
                        if ( uniqueNT || ! isupper ( front ) ) { // if new NT symbol or token, check if already mapped to goto
                            itgRetr = item_to_goto.insert ( STR_NUM_PAIR ( k.first, itemNum ) );
                            if ( itgRetr.second == false ) // already mapped
                                printf ( "   %-20s goto(%c)=I%d\n", k.first.c_str(), front, itgRetr.first->second );
                            else { // not mapped
                                orderList.push_back ( front ); // push in order List
                                printf( "   %-20s goto(%c)=I%d\n", k.first.c_str(), front, itemNum++ );
                            }
                        } else // not New NT symbol so just print
                            printf ( "   %-20s\n", k.first.c_str() );
            ...
            for ( auto& l : orderList ) //push new item lists in to the queue
